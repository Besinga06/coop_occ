    ...
    $('#form-new').validator().on('submit', function(e){
        ...
        $.ajax({
            ...
            success: function(msg){
                ...
                else { alert('Save failed.'); $('#btn-submit').prop('disabled', false); }
            },
            error: function(){ alert('Something went wrong!'); $('#btn-submit').prop('disabled', false);}
        });
        return false;
    });
});
...
// JS Functions
function closer(){ window.location.href='products.php'; }
function view_details(id){ window.location.href='product-details.php?product_id='+id; }
function auto_generate(){
    $.get('../transaction.php', { auto_generate: 1 }, function(msg){ $('#product-code').val(msg.trim()); });
}
function changePage(el){ $('#length_change').val($(el).attr('val')).trigger('change'); }
function checkProductDuplicate(){
    let name = $('#product_name').val().trim();
    if(name === '') return;
    $.get('../transaction.php', { checkproductExist: 1, product_name: name }, function(msg){
        msg = msg.trim();
        if(msg === '1'){
            $('#btn-submit').prop('disabled', true);
