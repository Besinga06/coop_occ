...
<style type="text/css">
    ...
    #show-search-member::-webkit-scrollbar-track { background-color: #F5F5F5; }
    #show-search-member::-webkit-scrollbar { width: 12px; background-color: #F5F5F5; }
    #show-search-member::-webkit-scrollbar-thumb { background-color: #3c8881; }
    .ul-search { list-style-type: none; background: #26a69a; color: #fff; margin-left: -25px; font-size: 12px; }
    .ul-search li { padding: 10px; height: 40px; font-size: 12px; cursor: pointer; border-bottom: 1px solid #dddddd; }
    #member-input { width: 200px; }
    #searchclearmember { position: absolute; right: 5px; top: 0; bottom: 0; height: 14px; margin: auto; font-size: 14px; cursor: pointer; color: #ccc; }
    .paging_simple_numbers, .dataTables_info { margin-top: 10px; }
</style>
...
if (!empty($_SESSION['loan-report-member'])) {
    ...
    $member_query = $db->query("SELECT name FROM tbl_customer WHERE cust_id=$member_id");
    $selected_member = $member_query ? $member_query->fetchArray(SQLITE3_ASSOC)['name'] : '';
}
...
if (!empty($_SESSION['loan-report-status'])) {
    ...
    $status_filter = " AND l.status='$status_val' ";
...
$summary = $summary_query ? $summary_query->fetchArray(SQLITE3_ASSOC) : [
    'total_loans'=>0,'total_requested'=>0,'total_approved'=>0,'total_disbursed'=>0
];
?>
...
.navbar-brand img { height: 40px!important; width: auto; object-fit: contain; }
    ...
    <div class="panel-body">
        <table class="table datatable-loan table-hover table-bordered">
            ...
            <tbody>
                ...
                while ($row = $loan_query->fetchArray(SQLITE3_ASSOC)) {

                    $paid_total = $db->querySingle("SELECT COALESCE(SUM(amount_paid),0) FROM tbl_loan_repayment WHERE loan_app_id = {$row['loan_app_id']}");
