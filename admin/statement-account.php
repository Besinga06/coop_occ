...
if(isset($_SESSION['soa-customer'])) { 
    ...
    $query = "SELECT * FROM tbl_sales  LEFT JOIN tbl_customer ON tbl_sales.cust_id=tbl_customer.cust_id  WHERE  tbl_sales.cust_id='".$cust_id."' AND balance > 0 GROUP BY sales_no";
					...
					<form class="heading-form" id="form-sales" method="POST">
						...
						<ul class="breadcrumb-elements" style="float:left">
                            ...
                            <li data-toggle="tooltip" title="Customer" style="padding-top: 2px;padding-right: 2px">
                                <div class="btn-group">
                                    <input autocomplete="off" type="hidden" value="<?php if (isset($_SESSION['soa-customer'])!="") { echo  $_SESSION['soa-customer']; } ?>" name="cust_id" id="cust_id">
                                    <input style="width: 230px"  autocomplete="off" type="search" class="form-control" id="customer-input" value="<?php if (isset($_SESSION['soa-customer'])!="") { echo  $selected_customer; } ?>" name="custname" >
                          ...
                          <?php if(isset($_SESSION['soa-customer'])) {  ?>
                           ...
                           <thead>
                                ...
                                ?>
                                    ...
                                    <?php if($i ==0) {?>
                                    <tr>
                                        <td colspan="6" class="center"><h3>No sales found!</h3></td>
                                    </tr>
                                <?php }?>
								</tbody>
                                <tfoot>
                           ...
                           <button  type="submit" class="btn bg-teal-400">Generate SOA</button>
                                ...
                                <?php }else{ ?>
                                <div class="text-center"><h3>Please select customer!</h3></div>
                            <?php } ?>
						</div>
				    </div>
...
function  view_details(el)
    ...
    function delivery_address(el){
       ...
       $.ajax({
           type      :      'POST',
           url       :      '../transaction.php',
           data      :       { update_delivery_address:'', sales_no: sales_no,  delivery_address: $(el).val()},
           success  :       function(msg)     
           {   
   ...
   function salesman(el){
      ...
      $.ajax({
          type      :      'POST',
          url       :      '../transaction.php',
          data      :       { update_salesman: '', sales_no: sales_no,  salesman: $(el).val()},
          success  :       function(msg)     
          {   
